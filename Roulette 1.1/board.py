...
#row 1
    for z in range(12):
        tk.Button(root,text=col+1,bg="red",fg="white",padx=20,pady=10,command=lambda rw=rw, col=col:bet_handler.ButtonPressed(rw,col)).grid(row=rw,column=col+2)
        col+=1
    rw += 1
    col = 0
#row 2
...
#row 3
    ...
    tk.Button(root, text="1st 12",pady=15,padx=60,bg="#bacc39",fg="white",command=lambda: test()).grid(row=4,column=2,columnspan=3)
    tk.Button(root, text="2nd 12",pady=15,padx=60,bg="#bacc39",fg="white",command=lambda: test()).grid(row=4,column=6,columnspan=3)
    tk.Button(root, text="3rd 12",pady=15,padx=60,bg="#bacc39",fg="white",command=lambda: test()).grid(row=4,column=10,columnspan=3)
    ...
    global pass_button
    pass_button = tk.Button(root, text="Pass Turn",state=tk.DISABLED,pady=14,padx=14,bg="#f48c42",fg="white",command=lambda: game.turn_change(game.turn_player))
