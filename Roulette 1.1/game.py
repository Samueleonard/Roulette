...
def gen_rand_int():
    ...
    if win_num in pd.player1.bets:
        pd.player1.total_chips -= (200*len(pd.player1.bets))
        print("PLAYER 1 won")
    if win_num in pd.player2.bets:
        pd.player2.total_chips -= (200*len(pd.player2.bets))
        print("PLAYER 2 won")
    

def turn_change(amt): # when the pass turn button is clicked
    print(amt)
    ply = amt
    if ply == 1:
